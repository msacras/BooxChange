...
import nl.booxchange.BooxchangeConstants
import nl.booxchange.Helper
...
class SignInActivity : BaseActivity(), OnCompleteListener<AuthResult> {
    ...
    private fun initializePhoneAuthorizationLayout() {
        ...
        phone_sign_in_button.setOnClickListener {
            val phoneNumber = (country_code_field.text.toString() + phone_number_field.text.toString()).takeIf { it.isNotBlank() } ?: return@setOnClickListener
            val imm = getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager
            imm.hideSoftInputFromWindow(phone_number_field.getWindowToken(), 0)
            hideViews {
    ...
    private fun getUserCountryCode(): String? {
        ...
        if (telephonyManager != null && !telephonyManager.simCountryIso.isEmpty()) {
            for (i in 0 until BooxchangeConstants.countries.size) {
                if (telephonyManager.simCountryIso.toUpperCase().equals(BooxchangeConstants.countries[i])) {
                    return Helper.addPlus(BooxchangeConstants.codes[i])
                }
            }
        }
        return getString(R.string.netherlands_country_code)
    }
