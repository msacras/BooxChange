...
import com.example.dima.booxchange.extension.*
  ...
  private fun initializeLayout() {
    ...
    search_cancel_button.setOnClickListener {
      if (isSearchOpen) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
          ValueAnimator.ofFloat(search_wrapper.elevation, dip(4f).toFloat()).apply {
            addUpdateListener {
              search_wrapper.elevation = (it.animatedValue as Float)
            }
            duration = 100
    ...
    offers_list_view.addOnScrollListener(object: RecyclerView.OnScrollListener() {
      ...
      override fun onScrollStateChanged(recyclerView: RecyclerView?, newState: Int) {
        ...
        if (newState == RecyclerView.SCROLL_STATE_IDLE && recyclerView.computeVerticalScrollRange() == recyclerView.computeVerticalScrollOffset() + recyclerView.computeVerticalScrollExtent()) {
